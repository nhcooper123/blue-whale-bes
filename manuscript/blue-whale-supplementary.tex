...
In temperate latitudes around the British Isles, seasonal dynamic cycles are present, but dampened (Fig. \ref{figs4}), whereas in subtropical regions exemplified by the Canaries and western Azores/Mid Atlantic ridge areas, $\delta^{13}$C values are relatively positive and constant throughout the year (Fig. \ref{figs4}).
  ...
  \begin{figure}[!htbp]
    \centering
      ...
      \caption{Alternative $\delta^{13}$C series arising from resident-only movement models restricting region to Norway, Ireland, Canaries and the Mid Atlantic ridge as labelled over a simulated two year period with daily resolution and 30 replicate simulations for each region.} % check legend
      \label{figs4}
  \end{figure}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[trim = 0cm 14cm 0cm 4cm, clip]{figures/flow-diagram-model.pdf}
